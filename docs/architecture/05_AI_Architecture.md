...
**AI proposes, a human decides, Workflow orchestrates, the domain executes.** Every consequential path involves up to four distinct roles that are never collapsed: AI is the proposing/advisory origin; a specific, identified human is the decision actor who approves, rejects, edits, or initiates the action directly; `Workflow` is the orchestration mechanism that sequences an approved request into execution; and the owning domain performs its own current authorization and business-rule validation and is the sole party that actually executes and records the business action. A human's approval of an AI proposal never converts AI into the acting or approving principal, and the resulting action is attributed to the human decision actor and the owning domain's execution — **it is never described as AI performing the action.**

**Non-delegable actions** fall into two kinds, neither weaker than the other. **Human-gated domain actions** — changing Matter status, assigning or removing lawyers, finalizing/signing/filing/sending/publishing/deleting a legal document, issuing or voiding an invoice, and changing identity, permissions, MFA, delegation, or privileged access — are actions a qualified human may still decide to take through the platform: AI may at most propose or draft, the human decides, Workflow may coordinate the approved request, and the owning domain authorizes and executes it. **Structural, absolute prohibitions** hold regardless of confidence, Firm configuration, automation-policy version, or any human-approval wording, because they describe what AI is and does rather than a delegation boundary a human decision can cross on AI's behalf: AI never presents generated content as legal advice or official law; AI never implies that interacting with it created an attorney-client relationship; AI never overrides or decides an Ethical Wall outcome; AI never acts as an authentication or authorization authority; **AI may never approve its own proposal or tool call, under any configuration**; AI never receives unrestricted shell, database, or credential access; AI never exposes restricted data or performs a destructive operation on its own initiative; and AI never broadens permissions or Client Portal audiences, approves or publishes Knowledge, removes confidentiality/privilege/retention/legal-hold restrictions, sends a substantive client communication outside an already-approved narrow Communications policy, alters rates or tax treatment, posts journals, allocates payments, writes off balances, reconciles accounts, or approves a refund or trust-to-operating transfer. **Client-money movement is absolute and categorically non-delegable to AI** — AI never initiates, approves, releases, or performs any client-money movement, regardless of human-approval wording, Firm configuration, or workflow design.
