...
- Playbooks are **guidance, not workflow execution.** This architecture defines the knowledge asset; it does not define a workflow engine, task automation, or execution state. `Workflow` (`docs/architecture/18_AI_Copilot_Workflow_Automation_Architecture.md`, ARCH-010) may *consume* an approved playbook through published contracts, but only via an explicit, governed human decision that converts it into an executable `WorkflowDefinition` — a playbook is never executable merely by existing, and that conversion is Workflow's own act, not part of this document (§K34).
...
- **Workflow consumption of playbooks** — `Workflow` (ARCH-010, `docs/architecture/18_AI_Copilot_Workflow_Automation_Architecture.md`) driving automation from approved playbooks through published contracts, via the explicit, governed conversion described in §K13. The conversion itself, and any resulting `WorkflowDefinition`, is Workflow's own architecture, not designed further here.
