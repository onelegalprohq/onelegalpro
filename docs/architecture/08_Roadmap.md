...
1. **Financial primitives, exact `Money`/currency, Firm accounting configuration, and policy versioning** — exact decimal `Money` with explicit `Currency`, `ExchangeRate` provenance, effective-dated jurisdiction and accounting policy, Firm financial configuration, Firm isolation.
