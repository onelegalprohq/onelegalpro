...
5. **Running workflows stay bound to their starting version.** A `WorkflowRun` remains bound to the exact `WorkflowVersion` it started with for its entire lifetime; publishing or editing a newer version never silently alters an already-running instance. Migrating a run to a newer version is an explicit, separately authorized human act.
...
- **Allow a newer published `WorkflowVersion` to automatically apply to already-running instances.** Rejected — this would silently change behavior underneath an already-approved, in-flight run; version migration is always an explicit, separately authorized act.
