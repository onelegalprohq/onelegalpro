...
- **Relying on identifier-only ordering — `ORDER BY id`, or a cursor over `id` alone — for event, business, chronological, causal, or delivery ordering**, because UUIDv7 is time-sortable. Rejected — it is the exact misreading Decision 4 exists to prevent, and it fails silently under same-millisecond generation, clock correction, concurrency, and commit reordering.
