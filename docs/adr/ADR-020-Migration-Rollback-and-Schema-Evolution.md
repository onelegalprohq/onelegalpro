...
11. **Module-owned migrations, with Platform Foundation owning platform infrastructure relations.** Each module's migrations live in its own `Database/` directory and touch **only** its own relations (`docs/domain/06_Laravel_Module_Blueprint.md`). Foundation — a shared technical-primitive layer, **not a bounded context** — owns both the outbox and the idempotency relations. **No module migrates, alters, reads, or writes another context's relations.**

    **Owning the relations is not owning the decisions about them, and the decisions are separate:** **`docs/adr/ADR-019-Transactional-Outbox-Persistence.md` owns transactional-outbox persistence only**; **`docs/adr/ADR-021-Idempotency-Persistence.md` owns idempotency persistence**; and **this ADR governs the migration and schema-evolution mechanics applicable to both**, on the same terms as any other relation. **Idempotency ownership is never attributed to ADR-019.**
...
13. **Every new relation declares one of the four classes** — **Firm-scoped, Firm-identifying, platform-global reference, or platform-realm security/operations** — in the migration that creates it (ADR-016 Decision 4), and a **Firm-scoped** relation ships with `firm_id NOT NULL`, Row-Level Security **enabled and forced**, and a policy carrying both `USING` and `WITH CHECK`, with **no role-based permissive exemption**. A **schema-level guard test enumerates relations** and asserts the declared class matches the relation's actual shape, so a new table cannot silently ship unprotected or misclassified (`docs/architecture/03_Database_Design.md` §16.3).

    **Two classes are closed by decision, not by convention:** a migration **never adds a second relation to the Firm-identifying class**, which contains the Firm registry alone; and a migration **never adds a relation to the platform-realm security/operations class without architecture and security approval**, that class presently containing the pre-authentication security-event relation alone.
...
- **No certification, compliance, production-readiness, or blanket legal-sufficiency claim is made.** The limited Thai-qualified review recorded in `docs/legal/ARCH-012-Thai-Legal-Review.md` occurred on 1 August 2026. An approved database design is **one** evidence item in `docs/architecture/02_Product_Requirements.md` §8; an approved deployment architecture, an **executed** restore test, monitoring, a documented incident procedure, the legal record's required follow-up policies and evidence, and the remaining approval gates are still outstanding. **No restore test has been executed.**
