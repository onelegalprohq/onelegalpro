...
2. **The bounded context contains three explicitly separated financial domain areas**: **Billing and Accounts Receivable**, **Client Money / Trust Accounting**, and **Firm Finance and Accounting**. They retain distinct aggregates, ledgers, lifecycles, permissions, and accounting meanings, while sharing two categories of common concept: the **platform-wide `Money`/`Currency` contract supplied by the Foundation Library (Decision 2a)**, which Billing consumes but does not own, and **Billing-owned financial-domain concepts and policies** — `ExchangeRate` provenance and application, `TaxTreatment`, `TaxIdentifier`, invoice numbering, allocation instructions, authorization records, beneficiary attribution, financial classification, and accounting and jurisdiction policy versions. The module keeps the name `Billing` because ARCH-003 through ARCH-006 already depend on "a future Billing bounded context" by that name.

   **2a. `Money` and its explicit `Currency` representation are a Foundation Library primitive governed by PF-045, not a Billing concept.** `AGENTS.md` and `docs/domain/06_Laravel_Module_Blueprint.md` place shared technical primitives in `app/Foundation`, and the approved Sprint Plan and Engineering Backlog already reserve **PF-045 — Money** in Sprint 0.3 (Foundation Library). **Billing consumes that Foundation contract and must never define a second, incompatible `Money` or `Currency` type**, nor may any other module. Exact-decimal representation, mandatory currency, and the prohibition on floating-point money are platform-wide Foundation guarantees; what Billing owns is the *financial-domain* meaning layered over them (Decision 2, above). This ADR neither schedules, renumbers, duplicates, nor marks PF-045 complete — it records the dependency.
...
9. **Financial values use the Foundation `Money` contract (PF-045): an exact decimal amount with an explicit `Currency`.** Floating-point money is prohibited everywhere in the platform. Billing consumes this contract rather than defining its own (Decision 2a).
...
16. **Client money and trust funds are segregated from Firm operating funds** in both accounting meaning and access controls. **Client money means money held by the Firm that is not the Firm's own money.** Who is beneficially entitled to a given balance — the client on the Matter, an identified third-party beneficiary, or a party pending an authorized entitlement determination — is a **separate, explicitly recorded fact**, never inferred from the depositor, the payer, the Primary `MatterClient`, or Matter membership. The architecture does not assume every balance is beneficially owned by the client named on the Matter. Accountable client/Matter subledger structure is retained in full; beneficial entitlement is recorded alongside it. A third-party beneficiary or payer referenced by a financial record gains no Matter, Client Portal, invoice, or document access by that reference.
...
18. **Every client-money receipt, disbursement, transfer, refund, and adjustment records** Firm, account, currency, amount, `BeneficiaryAttribution` (accountable client and/or authorized beneficiary reference, optional Matter attribution, purpose, and entitlement basis), Matter attribution where applicable, source, authorization, and audit provenance.
...
| `ExchangeRate` provenance and application, `TaxTreatment`, `TaxIdentifier`, invoice numbering, allocation instructions, authorization records, beneficiary attribution, financial classification, accounting and jurisdiction policy versions | **Billing** (shared across all three areas) | Owner — the financial-domain layer over the Foundation primitive |
| `Money`, `Currency` — the platform-wide exact-decimal money primitive | **Foundation Library (PF-045)** | **Consumed, never owned.** Billing must not define a second money or currency type |
...
- EPIC-008 depends on **EPIC-001 Platform Foundation, specifically PF-045 — Money**, for the platform-wide `Money`/`Currency` contract; on EPIC-006 (Client/Matter/MatterClient, Ethical Walls); EPIC-007 (rendered artifacts); EPIC-005 (Client Portal, Payment Widget); EPIC-004 (Invoice linking and delivery); and a future Identity/Security capability for actor identity and segregation-of-duties assignments. Its sequencing must account for all six. PF-045 is an existing approved backlog item; EPIC-008 depends on it without scheduling, duplicating, renumbering, or completing it.
...
- **Dependency outages are treated asymmetrically, by design.** A **Practice Management** outage **fails closed** for every operation requiring Matter validation, Matter authorization, `MatterClient` audience resolution, or `CheckEthicalWallAccess` — reads and writes alike — because Practice Management is the sole authority for those decisions (Decision 4). Such operations are rejected or left explicitly pending; Billing never approximates an authorization decision and never relies on a stale cached Ethical Wall result. Only independently valid local bookkeeping that records an already-authorized or already-committed fact, preserving the authorization provenance that permitted it, may continue. By contrast, a **Documents, Communications, or Digital Presence** outage must never rewrite or roll back an already issued or posted financial record — rendering, delivery, and integration work queues and retries, and outbound events wait in the approved outbox. **Availability never outranks authorization, Ethical Walls, or trust-account safeguards.**
