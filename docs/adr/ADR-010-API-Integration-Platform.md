...
- **Let Integrations implement payment-provider or messaging-provider webhook verification directly, to avoid duplicating shared verification logic.** Rejected as owner, though the underlying pattern (raw-body preservation, signature verification, replay windows) is intentionally shared — Billing and Communications still own their own provider adapters and provider-specific semantics; Integrations supplies the reusable ingress pattern, not the domain meaning of what was received.
