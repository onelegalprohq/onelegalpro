...
- **The concrete PlatformAdministration adapter** implementing PF-081's `CandidateFirmDirectory` port is `PA-007`'s. **Does not exist; not created by PF-081, and not created by `PA-001`.**
...
Architecture is **Approved** (ARCH-011 — Completed through explicit owner approval recorded on PR #30 on 29 July 2026). Implementation is **not scheduled**. The staged story list in `docs/architecture/08_Roadmap.md` (EPIC-012) remains **proposed only**, and **every story in this epic is `Backlog`** — none is Ready, In Progress, Code Review, Architecture Review, QA, Approved, or Done. Each requires its own approved entry here, with a Definition of Ready, before implementation begins.
...
**`PA-007` — Firm registry relation and `CandidateFirmDirectory` adapter — `Backlog`, no contract.** The ADR-016 Decision 4 class-(b) **Firm registry relation** — one row per Firm, no `firm_id`, identity/canonical name/jurisdiction/lifecycle metadata only — together with its Decision 4a access posture (dedicated database privileges, narrow named query contracts, application authorization; **no `FirmContext`-based Row-Level Security and no permissive `USING (true)` policy**), the narrow registry read, and the concrete adapter satisfying `PF-081`'s `CandidateFirmDirectory` port. **It is deliberately not one of the six approved ARCH-019 §28 stages**, and it is recorded separately because `PF-081` names the registry relation (B1) and the adapter (B4) as distinct blockers. **It is a database-design and security change** under the `AGENTS.md` gates and additionally depends on PostgreSQL role separation (`docs/adr/ADR-016-Tenant-Isolation-Model.md` Decision 10), none of which exists. Depends on `PA-001`.
