...
### PF-074 — PostgreSQL Runtime Role Separation — Backlog (scope outline; no full contract)

**Status.** `Backlog`. **Not** Ready, In Progress, Code Review, Architecture Review, QA, Approved, or Done. **No `PF-074` implementation has started and no `PF-074` file exists.** This is a **scope outline recorded on the technical owner's assignment of 13 August 2026**, not a full story contract: it fixes ownership, minimum scope, and exclusions so that `PA-007` has a named dependency rather than an unnamed one. **`PF-074` still requires its own full contract and its own Definition of Ready before implementation begins**, and **assigning the identifier schedules and authorizes nothing.**

**Objective.** Deliver the platform-wide PostgreSQL role separation that `docs/adr/ADR-016-Tenant-Isolation-Model.md` Decision 10 and `docs/architecture/03_Database_Design.md` §4.1 already require of every relation, so that least-privilege grants exist to be granted **before** any story creates a relation that depends on them. It is recorded here, in the Platform Foundation runtime lane, because the roles govern the whole database and are platform infrastructure — **not** one module's concern, and explicitly outside `PlatformAdministration`'s three permitted concepts (Constitution Article 45; `docs/adr/ADR-020-Migration-Rollback-and-Schema-Evolution.md` Decision 11).

**Why it is needed now.** The repository presently has **no role separation at all**: `config/database.php` holds one stock `pgsql` connection, and the required `Application Tests` job creates a single role that **owns the database, runs the migrations, and serves the requests** — precisely the configuration §4.1 prohibits ("A single database account used for migrations and for serving requests is prohibited"). `PA-007`'s registry deliberately carries **no** Row-Level Security policy, so dedicated privileges are its only database-level control; without `PF-074` that control cannot be delivered.

**Minimum scope.**

- a **migration role** owning DDL, never used to serve a request;
- an **application runtime role** with DML only — **not the owner, not a superuser, no `BYPASSRLS`, no DDL**;
- an **outbox publication role** *only to the extent accepted architecture already mandates it* (ADR-016 Decision 10, `docs/adr/ADR-019-Transactional-Outbox-Persistence.md` Decision 9, §4.1) — no capability beyond that is invented here;
- **migration and request-serving accounts are distinct**, provably so;
- the **runtime role is not the relation owner**;
- **no superuser attribute anywhere**;
- **no `BYPASSRLS` anywhere**;
- **least-privilege grants and the matching revocations**, including revoking whatever the default `PUBLIC` grants would otherwise confer;
- **CI and test provisioning** of those roles, and **PostgreSQL catalogue verification** asserting every property above against `pg_roles`, `pg_class`, and the privilege catalogue rather than against migration or workflow source.

**Explicit exclusions.** No `PA-007` schema, Firm registry relation, or any part of its implementation; no business relation of any kind; **no invented Row-Level Security policy** — `PF-074` grants and revokes privileges and creates no policy; no production secret or credential; no deployment, cloud-provider, or infrastructure-as-code configuration; no production provisioning; **no weakening of `PF-033`** — its PostgreSQL engine and non-superuser/`NOBYPASSRLS` guard must pass byte-identical, and the four required `Protect main` check names are unchanged; and no unrelated role, **no reporting role, no analytics role, and no owner exemption** (a cross-Firm reporting role does not exist, because no Reporting bounded context is approved — Constitution Article 44).

**Dependencies.** `PF-033` PostgreSQL Continuous Integration — **Done**. Approved architecture: ADR-016 Decisions 10 and 15; ADR-020 Decisions 5, 8, 11; `docs/architecture/03_Database_Design.md` §4.1, §15.5, §15.7; `docs/architecture/04_Security_Architecture.md`.

**Dependents.** `PA-007` (blocking); every later relation, grant, and Row-Level Security policy.

**Approval gates.** `PF-074` is a **database-design change and a security/authorization change** under `AGENTS.md`, and it touches `config/database.php`, `compose.yaml`, `.env.example`, and `.github/workflows/ci.yml`. Both gates apply, and its own contract must enumerate its allowed files before any implementation begins.
...
| The three platform-wide PostgreSQL roles and the migration/runtime account split | **`PF-074`** — assigned 13 August 2026; scope outline recorded, no full contract, `Backlog` | **No.** Named as a blocking dependency. |
