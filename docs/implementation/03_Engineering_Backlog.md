...
### PF-074 — PostgreSQL Runtime Role Separation — Backlog (Definition of Ready **met**)

**Status.** `Backlog`. **Definition of Ready met after independent contract review; implementation not authorized. Not** In Progress, Code Review, Architecture Review, QA, Approved, or Done. **No PF-074 implementation has started, no PF-074 production file exists, no PF-074 role has been created, and nothing is deployed.** This entry is the story **contract** only. Recording and approving a contract never schedules or starts a story; implementation requires the explicit database-design and security/authorization approval named below. The `PF-074` identifier was assigned by the technical owner on 13 August 2026; **assigning an identifier schedules and authorizes nothing**, and **no `PF-*` story is added, renamed, renumbered, merged, split, deleted, or rescheduled** by this contract.
...
## PA-007 — Firm Registry Relation and Narrow Registry Query — Backlog (Definition of Ready **not** met)

**Status.** `Backlog`. **Not** Ready, In Progress, Code Review, Architecture Review, QA, Approved, or Done. **No `PA-007` implementation has started**, no `PA-007` production file exists, and no relation, migration, database role, grant, privilege, or policy exists anywhere in this repository. Nothing is deployed. This entry is the story **contract** only; recording a contract never schedules or starts a story, and accepting this contract would **not** by itself make `PA-007` Ready — the Definition of Ready below is **not met** and names exactly why.
...
| The three platform-wide PostgreSQL roles and the migration/runtime account split | **`PF-074`** — full contract recorded and independently reviewed 14 August 2026; Definition of Ready met; implementation not authorized; `Backlog` | **No.** Named as a blocking dependency. |
...
1. **Dedicated database privileges — delivered by `PA-007`, conditional on `PF-074`.** The `GRANT` statements are `PA-007`'s and target the roles `PF-074` creates. Until `PF-074` is `Done`, this control cannot be delivered, which is why `PA-007`'s Definition of Ready is not met.
...
- **`PF-074` — PostgreSQL Runtime Role Separation — `Backlog`, full contract recorded and independently reviewed under EPIC-001 above; Definition of Ready met; implementation not authorized.** ADR-016 Decision 10 and `docs/architecture/03_Database_Design.md` §4.1 define three platform-wide logical roles — migration, application runtime, outbox publication — governing **every** relation, and prohibit "a single database account used for migrations and for serving requests". No such separation exists: `config/database.php` holds one stock `pgsql` connection with no role split, and the required `Application Tests` job creates one role that **owns the database, runs the migrations, and serves the requests** — exactly the prohibited configuration. **That is platform infrastructure, which ADR-020 Decision 11 assigns to Platform Foundation and which Constitution Article 45 places outside `PlatformAdministration`'s three permitted concepts.** Without it, compensating control 1 cannot be delivered. **`PA-007` may not begin until `PF-074` is `Done`.**
- **`PF-064` — Module Migration Registration — `Backlog`, contract recorded under EPIC-001 above.** ADR-020 Decision 11 and §5.1 require a module's migrations to live in its own `Database/` directory, and there is presently **no mechanism by which such a directory is discovered or run**. Placing the migration in `database/migrations/` instead would violate §5.1, and "historical migrations are never edited" makes a later relocation a new migration rather than a move. Inventing a registration mechanism inside `PA-007` would pre-create another story's type and would be exactly the invented standard `docs/PROJECT_STATUS.md`'s own rules forbid.
...
*Not resolved — why `PA-007` is not Ready:* **two mandatory dependencies do not exist**, and they are recorded as dependencies rather than worked around. `PA-007`'s Definition of Ready remains unmet **until both `PF-074` and `PF-064` are `Done`** — the two conditions the technical owner fixed on 13 August 2026. Without **`PF-074`**, compensating control 1 of the three ADR-016 Decision 4a calls mandatory cannot be delivered — and since the registry deliberately carries **no** Row-Level Security policy, privileges are the only database-level control it has, so shipping without them would leave the relation genuinely unrestricted at the database layer while this contract described it as restricted. Without **`PF-064`**, a module-owned migration has no discovery or execution path, and every available workaround violates an approved decision. **The two are independent of each other and may proceed in parallel.** Neither may be substituted by a stub, a single shared account, a temporary grant, a `USING (true)` policy, a root-directory migration, an ad-hoc service provider, or a `loadMigrationsFrom` call bolted onto an unrelated provider.
...
`PA-007` is itself preceded by **`PF-074`** PostgreSQL Runtime Role Separation and **`PF-064`** Module Migration Registration, both recorded under EPIC-001. Both must be `Done` before `PA-007`'s Definition of Ready can be met, and **the two are independent of each other, so they may proceed in parallel.** **`PA-001` is `Done` and precedes all of it.**
...
**Assigning `PF-074` and `PA-008` schedules and authorizes nothing.** Both are `Backlog` with scope outlines only; each requires its own full contract and its own Definition of Ready before implementation begins.
...
- **`PA-001` is `Done`; every other story in this epic is `Backlog`.** None of the others is Ready, In Progress, Code Review, Architecture Review, QA, Approved, or Done. **`PA-002` through `PA-006` carry reserved identifiers with no contract, no Definition of Ready, and no authorization.** **`PA-007` has a recorded contract but its Definition of Ready is *not* met** — it is blocked on **`PF-074`** and **`PF-064`**, which are independent of each other. **`PA-008` carries a recorded scope outline, not a full contract**, and is blocked on `PA-007` and `PF-081`. **Recording a contract or a scope outline, and assigning an identifier, schedules and authorizes nothing.** Nothing is deployed.
