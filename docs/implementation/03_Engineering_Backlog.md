...
The Foundation Library track has started. `PF-049`, `PF-047`, `PF-042`, `PF-048`, and `PF-044` are **Done**; `PF-041` is **In Progress** on branch `feature/pf-041-entity`, under its approved entry and Definition of Ready recorded below — **not Done, and its Definition of Done not yet met; it still requires pull-request review, all four required checks, a recorded human review comment, and merge** — followed by `PF-043`, which becomes next only once PF-041 completes. Every other story below remains Backlog — none is Ready, In Progress, or Done, and each still requires its own approved entry with a Definition of Ready before implementation begins.

- PF-040 AggregateRoot — Backlog
- PF-041 Entity — **In Progress** (branch `feature/pf-041-entity`; pending pull-request review, required checks, and merge before Done)
...
### PF-041 — Entity — In Progress

**Implementation status.** Implementation is present on branch `feature/pf-041-entity`. `Entity.php` and `EntityTest.php` exist, and the required local validation has passed: `composer pint:test`, `composer phpstan` (level 5, no errors), an informational level-6 dry run (no errors, no configuration change), the focused PF-041 suite, the full test suite, `composer validate --strict`, and both dependency audits have all been run in the canonical Docker PHP 8.4 container and passed. **PF-041 remains In Progress pending pull-request review, all four required checks, the recorded human review comment, and merge; it is not Done, and its Definition of Done below is not yet met.**
...
- `id()` returns the exact identifier instance supplied; identity cannot be replaced, including through reflection.
...
- **Identity:** `id()` returns the exact instance supplied (`assertSame`) and the concrete identifier type; reflection reassignment raises `\Error`; the forgetful fixture **constructs successfully and only fails when `id()` is reached** — asserted in that order, so the *latency* is what is pinned.
...
- **Identity is stable but not unforgeable.** `BusinessIdentifier::fromString()` permits reconstructing an *equal identifier value* from its canonical text at any time — it says nothing about whether an *entity* carrying that identifier can be constructed, which remains the owning module's own construction rule. "Same identity" proves nothing about provenance, authenticity, ownership, authorization, or entitlement.
...
**Definition of Done.** Acceptance criteria met; `composer validate --strict`, `composer pint:test`, `composer phpstan` (level 5, no baseline, suppression, or ignored error), the focused PF-041 suite, the full test suite, `composer audit --locked --abandoned=report`, and `npm audit --audit-level=high` all executed in the canonical Docker PHP 8.4 application container and passing, with real reported counts; the informational level-6 dry run recorded without any configuration change; the PF-049 architecture guard passed unchanged; security and architecture reviewed; documentation updated; no critical defect; human approval recorded on the pull request before merge.
