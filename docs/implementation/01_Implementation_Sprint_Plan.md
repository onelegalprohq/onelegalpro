...
**ARCH-011 — Release 0.1 Rescope, Platform Administration Ownership, and Deferred-Control Decisions: architecture Proposed (In Review — not Completed, not approved).** See `docs/architecture/01_OneLegalPro_Constitution.md` (Articles 45–48, **Proposed**), `docs/architecture/02_Product_Requirements.md` (populated as the **release-scoped** Release 0.1 product requirements, Proposed), `docs/architecture/08_Roadmap.md` (EPIC-012), `docs/architecture/19_Platform_Administration_Architecture.md`, and `docs/adr/ADR-012-Release-0-1-Product-Scope-and-Matter-Desk-Slice.md`, `docs/adr/ADR-013-Firm-Provisioning-and-Subscription-Entitlement-Ownership.md`, `docs/adr/ADR-014-Operator-Assisted-Onboarding-and-Privileged-Access.md`, and `docs/adr/ADR-015-Deferred-Professional-Responsibility-Controls.md` — **all four ADRs are Proposed, none is Accepted.** Establishes `PlatformAdministration` as a **narrowly bounded supporting context owning exactly `Firm`, `FirmProvisioning`, and `SubscriptionEntitlement`**, resolving the `Firm` ownership gap that IdentityAccess, Platform Foundation, Branding, and every Firm-scoped context referenced but none owned; records **Release 0.1 — the OneLegalPro Matter Desk** as a founding-firm pilot slice; and records the deferral of **Ethical Walls and automated conflict checking**, which are **absent, disclosed, and never stubbed or approximated**, with a **manual conflict attestation** satisfying the `Opened` gate as a human determination that is never described as a system-performed conflict check. **Billing remains the sole owner of Firm-to-client commercial and financial records; `SubscriptionEntitlement` carries no monetary value; Constitution Articles 1–44 are unchanged and unweakened.** Unlike ARCH-001 through ARCH-010, this story is **not Completed**: it awaits explicit human review and approval. **`docs/architecture/03_Database_Design.md` and `docs/architecture/06_Marketplace.md` remain empty placeholders**, and `docs/architecture/04_Security_Architecture.md`, `docs/architecture/05_AI_Architecture.md`, and `docs/architecture/07_API_Standards.md` were **not modified** — no new AI capability, no API change, and no security-baseline change is introduced. The story ID is **ARCH-011** while the architecture document is numbered **19** (`ARCH-019`), continuing the established distinction between story numbering and architecture-document numbering.
