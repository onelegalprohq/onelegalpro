...
**ARCH-012 — Data & Persistence Architecture: review-complete, Proposed, ready for repository-owner approval.** The story populates `docs/architecture/03_Database_Design.md` and proposes ADR-016 through ADR-021. Independent architecture review and correction passes are complete; the eight legal questions raised by the architecture received Thai-qualified review and owner-approved decisions recorded separately in `docs/legal/ARCH-012-Thai-Legal-Review.md`. Until explicit repository-owner approval is recorded on the ARCH-012 pull request, the architecture and all six ADRs remain Proposed and authorize no implementation, deployment, or production access. Approval will not itself schedule PF-033, PF-080, or any other implementation story.
