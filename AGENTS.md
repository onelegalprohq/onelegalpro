...
- **`Money` is an exact decimal amount with an explicit currency.** Floating-point money is prohibited. Multi-currency amounts record transaction currency, base currency, rate, rate source, and rate timestamp; historical conversions are never recalculated with a newer rate.
