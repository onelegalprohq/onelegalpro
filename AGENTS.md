...
- Documents is its own bounded context and the sole owner of the canonical document record and every stored document version. Stored versions are immutable — a correction or replacement creates a new version, preserving historical bytes, checksums, authorship, timestamps, and provenance. See `docs/architecture/01_OneLegalPro_Constitution.md` (Articles 18–19), `docs/architecture/14_Document_Management_Architecture.md`, and `docs/adr/ADR-007-Document-Management.md`.
