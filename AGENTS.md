...
- **Published workflow versions are immutable, and a running workflow stays bound to the exact version it started with.** Publishing or editing a newer version never silently alters an already-running instance; migrating a run to a newer version is an explicit, separately authorized human act.
...
- **Some actions are non-delegable to AI regardless of Firm configuration** — including changing Matter status, overriding an Ethical Wall, issuing/voiding an invoice, moving client money, altering identity/permissions/MFA, finalizing or sending a legal document, and approving or publishing Knowledge. The full list is in `docs/architecture/18_AI_Copilot_Workflow_Automation_Architecture.md` §11. **Financial calculations remain deterministic outside the model, and client-money movement is never AI-initiated, AI-approved, or AI-released.**
