...
> **Status: Proposed, not yet approved (ARCH-011).** The rules in this section and the next derive from `docs/architecture/01_OneLegalPro_Constitution.md` Articles 45–48 and from `docs/adr/ADR-012-Release-0-1-Product-Scope-and-Matter-Desk-Slice.md`, `docs/adr/ADR-013-Firm-Provisioning-and-Subscription-Entitlement-Ownership.md`, `docs/adr/ADR-014-Operator-Assisted-Onboarding-and-Privileged-Access.md`, and `docs/adr/ADR-015-Deferred-Professional-Responsibility-Controls.md` — **all Proposed, none Accepted**. They authorize no implementation. Where they touch an approved rule elsewhere in this file, **the approved rule governs**; nothing here weakens one.
...
- **Entitlement is not authorization.** A valid entitlement grants nothing by itself; IdentityAccess composes it at the authentication and membership boundaries, and it can only narrow. Entitlement decisions **fail closed** — when entitlement state cannot be authoritatively determined, new authentication is refused.
...
- **There is no Firm-level suspended state.** A `Firm`'s lifecycle is `Requested → Provisioned → Active → Closed`, and **neither suspension meaning above is a Firm-wide disable**. **Never invent one** — a future Firm-level suspension or emergency-disable capability requires its own separately approved decision defining the authorizing authority, session effects, recovery, notification, queued-and-in-flight-work behaviour, and audit semantics distinguishing it from both entitlement lapse and membership revocation.
